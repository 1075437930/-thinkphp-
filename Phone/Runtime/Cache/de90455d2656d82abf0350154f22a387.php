...
<div class="backtop"></div>
    ...
    <div class="container">
        <article>
            <h1>我的收件箱</h1>
            <section>
                <h2 class="op"><span>回复通知</span></h2>
                <?php if(!empty($res)): if(is_array($res)): foreach($res as $key=>$row): ?><div class="oneitem">
                            <p class="fir"><span><?php echo ($row['username']); ?></span><time><?php echo date('m-d H:i',$row['rt']);?></time></p>
                            <p>
                                <span>在</span>
                                <a href="__APP__/Article/artinfo/id/<?php echo ($row['ti']); ?>"><?php echo ($row['tt']); ?></a>
                                <span>回复了你:</span>
                            </p>
                            <p><span>@<?php echo ($_SESSION['username']); ?></span><span><?php echo ($row['rc']); ?></span></p>
                            <div class="close" di="<?php echo ($row['ri']); ?>">X</div>
